...
my_smry_df <- palmerpenguins::penguins %>% 
  group_by(species) %>% 
  summarize(across(where(is.numeric),.fns =  \(x) mean(x, na.rm = TRUE))) %>% 
  select(-year) 
...
these_findings <- analyzer_chat$extract_data(
  create_analysis_prompt(my_smry_df), 
...
these_findings$headline
these_findings$findings
